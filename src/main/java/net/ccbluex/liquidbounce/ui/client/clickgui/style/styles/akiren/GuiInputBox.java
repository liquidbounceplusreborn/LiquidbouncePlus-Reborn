...
public class GuiInputBox extends Gui {
    private final int id;
    private final GameFontRenderer fontRendererInstance;
    /**
    ...
    public GuiInputBox(int componentId, GameFontRenderer fontrendererObj, int x, int y, int par5Width, int par6Height) {
        this.id = componentId;
    ...
    public void drawTextBox() {
        if (this.getVisible()) {
            if (this.getEnableBackgroundDrawing()) {
                drawRect(this.xPosition, this.yPosition, this.xPosition + this.width, this.yPosition + this.height, new Color(0, 0, 0, 141).getRGB());
            }

            int i = this.isEnabled ? this.enabledColor : this.disabledColor;
            int j = this.cursorPosition - this.lineScrollOffset;
            int k = this.selectionEnd - this.lineScrollOffset;
            String s = Minecraft.getMinecraft().fontRendererObj.trimStringToWidth((text.isEmpty() ? placeholder : text).substring(this.lineScrollOffset), this.getWidth());
            ...
            if (s.length() > 0) {
                ...
                Fonts.fontSFUI35.drawString(s1, l, i1, i);
                j1 += Fonts.fontSFUI35.getStringWidth(s1);
            }
            ...
            if (s.length() > 0 && flag && j < s.length()) {
                Fonts.fontSFUI35.drawString(s.substring(j), j1, i1, i);
            }

            if (flag1) {
                if (flag2) {
                    Gui.drawRect(k1, i1 - 1, k1 + 1, i1 + 1 + Fonts.fontSFUI35.getHeight(), -3092272);
                } else {
                    Fonts.fontSFUI35.drawString("_", k1, i1, i);
                }
            }

            if (k != j) {
                int l1 = l + this.fontRendererInstance.getStringWidth(s.substring(0, k));
                this.drawCursorVertical(k1, i1 - 1, l1 - 1, i1 + 1 + Fonts.fontSFUI35.getHeight());
            }
        }
    }
