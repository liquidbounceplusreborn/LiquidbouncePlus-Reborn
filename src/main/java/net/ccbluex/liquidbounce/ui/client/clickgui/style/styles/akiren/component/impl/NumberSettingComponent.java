...
public class NumberSettingComponent extends Component implements PropertyComponent {
    ...
    public IntegerValue numberSetting;
    public float currentValueAnimate = 0f;
    private boolean sliding;
    Minecraft mc = Minecraft.getMinecraft();
    public NumberSettingComponent(Component parent, IntegerValue numberSetting, int x, int y, int width, int height) {
    ...
    public void drawComponent(ScaledResolution scaledResolution, int mouseX, int mouseY) {
        ...
        if (this.sliding) {
            numberSetting.set((float) MathUtils.round((double) (mouseX - x) * (max - min) / (double) width + min, 1));
        ...
        float optionValue = (float) MathUtils.round(numberSetting.get(), 1f);
    ...
    @Override
    public Value<Integer> getSetting() {
