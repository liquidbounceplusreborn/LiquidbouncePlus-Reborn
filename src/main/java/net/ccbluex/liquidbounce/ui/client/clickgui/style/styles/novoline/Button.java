...
public class Button {
    ...
    public void render(int mouseX, int mouseY, int x11,int y11,int x22,int y22) {
        final GameFontRenderer font = Fonts.fontSFUI35;
        float y2 = y + 15;
        buttons.clear();
        ...
        if (!expand && size >= 1) {
            Fonts.fontSFUI35.drawString("+", x + 75 + parent.allX, y - 1, -1);
        } else if (size >= 1) {
            //RenderUtils.drawRect(x - 5, y - 5, x + 85 + parent.allX, y + font.FONT_HEIGHT + 5, new Color(40, 40, 40).getRGB());
            Fonts.fontSFUI35.drawString("-", x + 75 + parent.allX, y - 1, -1);
        }
    ...
    public void click(int mouseX, int mouseY, int button) {
        ...
        if (mouseX > x - 7 && mouseX < x + 85 + parent.allX && mouseY > y - 6 && mouseY < y + Fonts.fontSFUI35.FONT_HEIGHT) {
            if (button == 0) {
