...
object GuiCape : GuiScreen() {
    private val jsonFile = File(LiquidBounce.fileManager.capesDir, "cape.json")

    private val embeddedCapes = mutableListOf<ICape>()

//    private val embeddedCape = mutableListOf<ICape>()

    var nowCape: ICape?
    val capeList = mutableListOf<ICape>()

    init {
        arrayOf("dark", "darker", "light", "special1", "special2").forEach {
                embeddedCapes.add(loadCapeFromResource(it, "assets/minecraft/liquidbounce+/cape/$it.png"))
            }
            arrayOf("ghost", "red", "rgb", "purple", "rose", "glitchstorm", "rickroll", "lightning").forEach {
                embeddedCapes.add(loadGifCapeFromResource(it, "assets/minecraft/liquidbounce+/cape/$it.gif"))
            }
    ...
    private fun loadCapeFromResource(name: String, loc: String) = SingleImageCape(name, ImageIO.read(GuiCape::class.java.classLoader.getResourceAsStream(loc)))

    private fun loadCapeFromFile(name: String, file: File) = SingleImageCape(name, ImageIO.read(file))

    private fun loadGifCapeFromResource(name: String, loc: String) = GifCape(name, GuiCape::class.java.classLoader.getResourceAsStream(loc))
    ...
    override fun initGui() {
        this.buttonList.add(GuiButton(1, (width * 0.3).toInt(), (height * 0.5).toInt(), Fonts.fontTahoma30.getStringWidth("<") + 10, 20, "<"))
        this.buttonList.add(GuiButton(2, (width * 0.7).toInt(), (height * 0.5).toInt(), Fonts.fontTahoma30.getStringWidth(">") + 10, 20, ">"))
    }
    ...
    override fun drawScreen(p_drawScreen_1_: Int, p_drawScreen_2_: Int, p_drawScreen_3_: Float) {
        ...
        GL11.glPushMatrix()
        Fonts.font35.drawCenteredString(if (nowCape == null) { "§cNONE" } else { "§a${nowCape!!.name}" }, width * 0.50f, height * 0.23f, -1, false)
        GL11.glScalef(2f, 2f, 2f)
        Fonts.font35.drawCenteredString("Cape", width * 0.25f, height * 0.03f, -1, false)
        GL11.glPopMatrix()
