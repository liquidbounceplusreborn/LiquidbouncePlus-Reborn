...
@ModuleInfo(name = "AuthBypass", spacedName = "Auth Bypass", description = "Bypass auth when join server.", category = ModuleCategory.WORLD)
class AuthBypass : Module() {
    ...
    fun onUpdate(event: UpdateEvent) {
        if (packets.isNotEmpty() && timer.hasTimePassed(delayValue.get().toLong())) {
            ...
            packets.clear()
            LiquidBounce.hud.addNotification(Notification("AuthBypass","Authentication bypassed.", NotifyType.INFO))
        }
    }
