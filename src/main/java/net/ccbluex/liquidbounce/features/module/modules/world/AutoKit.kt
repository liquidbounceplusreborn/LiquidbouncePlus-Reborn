...
class AutoKit : Module() {
    ...
    private val editMode: BoolValue = object : BoolValue("Edit-Mode", false) {
        override fun onChanged(oldValue: Boolean, newValue: Boolean) {
            if (newValue)
                LiquidBounce.hud.addNotification(Notification(name,"Change default kit by right clicking the kit selector and select.", Type.INFO))
        }
    }
    ...
    fun onUpdate(event: UpdateEvent) {
        ...
        if (clickStage == 2) {
            ...
            if (timeoutTimer.hasTimePassed(kitTimeOutValue.get())) {
                ...
                LiquidBounce.hud.addNotification(Notification(name,"Kit checker timed out. Please use the right kit name.", Type.ERROR))
    ...
    fun onPacket(event: PacketEvent) {
        ...
        if (packet is C0DPacketCloseWindow && editMode.get()) {
            editMode.set(false)
            LiquidBounce.hud.addNotification(Notification(name,"Edit mode aborted.", Type.INFO))
        ...
        if (packet is S02PacketChat) {
            ...
            if (text.contains("kit has been selected", true)) {
                if (editMode.get()) {
                    ...
                    clickStage = 0
                    listening = false
                    LiquidBounce.hud.addNotification(Notification(name,"Successfully detected and added $kitName kit.", Type.SUCCESS))
                    debug("finished detecting kit")
                    return
                } else {
                    listening = false
                    event.cancelEvent()
                    LiquidBounce.hud.addNotification(Notification(name,"Successfully selected ${kitNameValue.get()} kit.", Type.SUCCESS))
                    debug("finished")
                    return
                }
            }
