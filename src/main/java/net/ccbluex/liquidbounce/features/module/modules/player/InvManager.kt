...
class InvManager : Module() {
    ...
    private val items = arrayOf("None", "Ignore", "Sword", "Bow", "Pickaxe", "Axe", "Food", "Block", "Water", "Gapple", "Pearl", "Potion","Shovel")
    ...
    fun performManager() {
        if ((noScaffoldValue.get() && (LiquidBounce.moduleManager[Scaffold::class.java]!!.state || LiquidBounce.moduleManager[Scaffold2::class.java]!!.state))|| !InventoryUtils.CLICK_TIMER.hasTimePassed(delay) ||
            ...
            if (item is ItemSword || item is ItemTool) {
                ...
                for (i in 0..8) {
                    if (type(i).equals("sword", true) && item is ItemSword ||
                        ...
                        type(i).equals("Shovel", true) && item is ItemSpade ||
        ...
        when (type.toLowerCase()) {
            "sword", "pickaxe", "shovel", "axe" -> {
                val currentType: Class<out Item> = when {
                    ...
                    type.equals("Shovel", ignoreCase = true) -> ItemSpade::class.java
