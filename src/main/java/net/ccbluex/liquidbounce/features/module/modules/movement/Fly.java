        ...
        switch (mode.toLowerCase()) {
            ...
            case "watchdog":
                ...
                if (expectItemStack == -1) {
                    LiquidBounce.hud.addNotification(new Notification("Fly","The fly requires slime blocks to be activated properly.", NotifyType.ERROR, 1500, 500));
                    break;
                }
    ...
    public void onUpdate(final UpdateEvent event) {
        ...
        switch (modeValue.get().toLowerCase()) {
            ...
            case "pearl":
                ...
                if (pearlState == 0) {
                    if (enderPearlSlot == -1) {
                        LiquidBounce.hud.addNotification(new Notification("Fly","You don't have any ender pearl!", NotifyType.ERROR, 1500, 500));
                        pearlState = -1;
                        this.setState(false);
                        return;
                    }
    ...
    public void onMotion(final MotionEvent event) {
        ...
        if (modeValue.isMode("GrimTNT")) {
            ...
            if(event.getEventState() == EventState.PRE) {
                    mc.thePlayer.setPositionAndRotation(mc.thePlayer.posX + 1000, mc.thePlayer.posY, mc.thePlayer.posZ, mc.thePlayer.rotationYaw, mc.thePlayer.rotationPitch);
                }
