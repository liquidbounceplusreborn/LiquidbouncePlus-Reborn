...
@ModuleInfo(name = "AntiBot", spacedName = "Anti Bot", category = ModuleCategory.MOVEMENT, description = "Prevent you from getting slowed down by items (swords, foods, etc.) and liquids.")
object AntiBot : Module() {
    ...
    private val livingTimeTicksValue = IntegerValue("LivingTimeTicks", 40, 1, 200,{ livingTimeValue.get() })
    ...
    private val removeIntervalValue = IntegerValue("Remove-Interval", 20, 1, 100,{ removeFromWorld.get() })
    ...
    private val alwaysInRadiusWithTicksCheckValue = BoolValue("AlwaysInRadiusWithTicksCheck", false,{ alwaysInRadiusValue.get() && livingTimeValue.get() })
    ...
    fun isBot(entity: EntityLivingBase): Boolean {
        ...
        if (tabValue.get()) {
            ...
            val targetName = ColorUtils.stripColor(entity.displayName.formattedText)

            for (networkPlayerInfo in mc.netHandler.playerInfoMap) {
                val networkName = ColorUtils.stripColor(networkPlayerInfo.getFullName())
