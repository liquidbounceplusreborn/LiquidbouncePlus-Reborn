...
public class Window {
    ...
    public void render(int mouseX, int mouseY) {
        ...
        RenderUtils.drawBorderedRect(x - 0.5f, y - 0.5f, x + 91f + allX, y + expand, 0.05f, new Color(29, 29, 29).getRGB(), new Color(40, 40, 40).getRGB());

        if (category.name().equals("Misc")) {
            Fonts.fontSFUI35.drawStringWithShadow("Exploit", x + 4, y + 5, -1);
        }
        if (category.name().equals("World")) {
            Fonts.fontSFUI35.drawStringWithShadow("Misc", x + 4, y + 5, -1);
        }
        if (category.name().equals("Render")) {
            Fonts.fontSFUI35.drawStringWithShadow("Visuals", x + 4, y + 5, -1);
        }
        if (!category.name().equals("Render") && !category.name().equals("Misc") && !category.name().equals("World")) {
            Fonts.fontSFUI35.drawStringWithShadow(category.name(), x + 4, y + 5, -1);
        }
        //Icons start
       // final GameFontRenderer novoicons = Fonts.fontNovoIcon20;
        final GameFontRenderer novoicons = Fonts.fontSFUI35;
        if (category.name().equals("Combat")) {
